...
int main(){
    ...
    const auto aspecto = 16.0 / 9.0; //Formato da imagem. Exemplos:  16:9, 4:3 e etc...
    ...
    auto matChao = std::make_shared<lambertian>(cor(0.5, 0.3, 0.4));
    auto matCen = std::make_shared<metal>(cor(1, 1, 1), 0);
    auto matDir = std::make_shared<lambertian>(cor(0.7, 1, 0.3));
    auto matEsq = std::make_shared<dieletrico>(1.5);
    auto matInf = std::make_shared<dieletrico>(1.3);
    auto matSup = std::make_shared<metal>(cor(0.8, 0.6, 0.2), 0);

    mundo.add(std::make_shared<esfera>(ponto3D( 0.0, -100.5, -1.0), 100.0, matChao));
    mundo.add(std::make_shared<esfera>(ponto3D( 0.0,  0.85,   -2.0),   0.5, matCen));
    mundo.add(std::make_shared<esfera>(ponto3D(-1.0,    1,   -1.0),   0.5, matEsq));
    mundo.add(std::make_shared<esfera>(ponto3D( 1.0,    1,   -1.0),   0.5, matDir));
    mundo.add(std::make_shared<esfera>(ponto3D( 0,      2,     -1),   0.5, matSup));
    mundo.add(std::make_shared<esfera>(ponto3D( 0,      0.0, -1.0),   0.5, matInf));
    
    //Câmera
    ponto3D origem(0,2,4);
    ponto3D olharPara(0, 1, 0);
    vec3 vecCima(0,1,0);
    auto distParaFoco = 7;
    auto abertura = 0.1;
    camera cam(50.0, aspecto, origem, olharPara, vecCima, abertura, distParaFoco);
